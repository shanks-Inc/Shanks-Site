      ...
      <header className="sticky top-0 z-50 border-b border-neutral-800 backdrop-blur bg-black/50">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
          <a href="#home" className="flex items-center gap-3">
            <img src={LOGO} alt="Shanks" className="h-9 w-auto drop-shadow" />
          </a>
          <nav className="hidden md:flex items-center gap-6 text-sm">
            {nav.map(n => (
              <a key={n.href} href={n.href} className="opacity-85 hover:opacity-100">{n.label}</a>
            ))}
            <a href="#contact" className="rounded-2xl px-4 py-2 bg-[var(--gold)] text-black font-semibold shadow-glow">
              Bookings
            </a>
          </nav>
        ...
        {open && (
          <div className="md:hidden border-t border-neutral-800 bg-brand-black">
            <div className="px-4 py-3 space-y-2">
              ...
              <a href="#contact" className="block rounded-xl px-3 py-2 bg-[var(--gold)] text-black font-semibold">
                Bookings
              </a>
            </div>
          </div>
          ...
          <div className="mt-6 grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {GALLERY.map((src,i)=>(
              <figure key={i} className="overflow-hidden rounded-2xl border border-neutral-800 group">
                <img src={src} alt={`Shanks gallery ${i+1}`} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300" />
              </figure>
            ))}
          </div>
        </div>
